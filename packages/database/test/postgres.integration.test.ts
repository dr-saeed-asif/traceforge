...
import { PostgresEventRepository, runMigrations } from "../src/index.js";

const connectionString = process.env.TEST_DATABASE_URL;
const databaseDescribe = connectionString === undefined ? describe.skip : describe;

databaseDescribe("PostgreSQL adapters", () => {
  let pool: Pool;
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `INSERT INTO tasks (task_id,title,repository,workspace,created_at,created_by,status)
       VALUES ($1,'Integration task','repo','workspace',now(),'test','IN_PROGRESS')`, [taskId]
    );
    await pool.query(
      `INSERT INTO sessions (session_id,task_id,started_at,developer) VALUES ($1,$2,now(),'test')`,
      [sessionId, taskId]
    );
    await pool.query(
      `INSERT INTO agent_runs (run_id,session_id,agent_id,agent_name,agent_version,status,started_at)
       VALUES ($1,$2,'agent','agent','1.0.0','RUNNING',now())`, [runId, sessionId]
    );
  ...
  afterAll(async () => {
    if (pool !== undefined) {
      ...
      await pool.query("DELETE FROM ingress_receipts WHERE event_id IN (SELECT event_id FROM provenance_events WHERE run_id = $1)", [runId]);
      await pool.query("DELETE FROM provenance_events WHERE run_id = $1", [runId]);
  ...
  function event(sequence: number, previousEventHash: string | null) {
    const draft: UnhashedProvenanceEvent = {
      ...
      source: { adapter: "integration", adapterVersion: "1.0.0", evidence: "OBSERVED" },
  ...
  it("rejects a stale sequence and emits transactional outbox records", async () => {
    ...
    const messages=await pool.query("SELECT message_id FROM outbox_events WHERE aggregate_id=$1 AND topic='provenance.event.appended'",[runId]);
